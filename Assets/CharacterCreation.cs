...
public class CharacterCreation : MonoBehaviour
{
    ...
    public void CreateCharacter()
    {
        if (CharacterValidation())
        {
            ...
            character.CharacterData(0, playerName.text,characterName.text, int.Parse(characterHP.text), int.Parse(characterInitiative.text), characterRace.options[characterRace.value].text, characterClass.options[characterClass.value].text);
